...
SkinConfig::SkinConfig(const String& skin)
{
	...
	File defFile;
	String configPath = Path::Normalize(Path::Absolute("/storage/emulated/0/.usc/skins/" + skin + "/config-definitions.json"));
	//Log(configPath);
    ...
    InitDefaults();
	Load(Path::Normalize(Path::Absolute("/storage/emulated/0/.usc/skins/" + skin + "/skin.cfg")));
}
...
SkinConfig::~SkinConfig()
{
	...
	Save(Path::Normalize(Path::Absolute("/storage/emulated/0/.usc/skins/" + m_skin + "/skin.cfg")));
