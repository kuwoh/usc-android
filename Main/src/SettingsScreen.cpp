...
private:
	...
	void m_pruneReplaysImpl(float delta)
	{
		...
		for (; m_replayPruneIter != end && count < 100; m_replayPruneIter++)
		{
			...
			String replayPath = Path::Normalize("/sdcard/usc/replays/" + chart->hash + "/");
...
protected:
	void Load() override
	{
		...
		m_allSkins = Path::GetSubDirs(Path::Normalize("/sdcard/usc/skins/"));
	...
	void RenderContents() override
	{
		...
		{
			Image cursorImg = ImageRes::Create("/sdcard/usc/skins/" + g_gameConfig.GetString(GameConfigKeys::Skin) + "/textures/cursor.png");
