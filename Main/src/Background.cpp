...
public:
	virtual bool Init(bool foreground) override
	{
		...
		defaultBGs = Path::GetSubDirs(Path::Normalize(
			Path::Absolute("sdcard0/.usc/skins/" + g_application->GetCurrentSkin() + "/backgrounds/")));
		...
		path = Path::Absolute("sdcard0/.usc/skins/" + skin + "/backgrounds/fallback/");
		folderPath = path;
	...
	{
		String skin = g_gameConfig.GetString(GameConfigKeys::Skin);
		String pathV = Path::Absolute(String("sdcard0/.usc/skins/" + skin + "/shaders/") + "background" + ".vs");
		String pathF = Path::Absolute(path);
		String pathG = Path::Absolute(String("sdcard0/.usc/skins/" + skin + "/shaders/") + "background" + ".gs");
