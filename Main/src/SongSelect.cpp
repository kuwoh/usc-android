...
public:
	...
	bool Init() override
	{
		...
		CheckedLoad(m_lua = g_application->LoadScript("sdcard0/.usc/songselect/songwheel"));
		lua_newtable(m_lua);
		{
			//text
	...
	{
		g_application->ReloadScript("sdcard0/.usc/songselect/songwheel", m_lua);
	...
	void Render(float deltaTime) override
	{
		...
		{
			g_application->ScriptError("sdcard0/.usc/songselect/songwheel", m_lua);
		}
	}
...
public:
	...
	bool Init()
	{
		...
		CheckedLoad(m_lua = g_application->LoadScript("sdcard0/.usc/songselect/filterwheel"));
		return true;
	}
	void ReloadScript()
	{
		g_application->ReloadScript("sdcard0/.usc/songselect/filterwheel", m_lua);
	}
	void Render(float deltaTime)
	{
		...
		{
			g_application->ScriptError("sdcard0/.usc/songselect/filterwheel", m_lua);
		}
	}
	~FilterSelection()
	{
...
public:
	...
	bool Init()
	{
		...
		CheckedLoad(m_lua = g_application->LoadScript("sdcard0/.usc/songselect/sortwheel"));
		m_SetLuaTable();
	...
	void SetSelection(SongSort *s)
	{
		...
		if (lua_pcall(m_lua, 1, 0, 0) != 0)
		{
			g_application->ScriptError("sdcard0/.usc/songselect/sortwheel", m_lua);
		}
	}
		...
		else
		{
			...
			else if (code == SDL_SCANCODE_F12 && m_shiftDown)
			{
				...
				if (sel) {
					...
					String replayPath = Path::Normalize(Path::Absolute("sdcard0/.usc/replays/" + hash + "/"));
