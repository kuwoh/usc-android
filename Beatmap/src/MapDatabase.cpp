...
public:
	MapDatabase_Impl(MapDatabase& outer, bool transferScores) : m_outer(outer)
	{
		...
		String databasePath = Path::Absolute("/storage/emulated/0/.usc/maps.db");
		...
		else if (update)
		{
			...
			//back up old db file
			Path::Copy(Path::Absolute("/storage/emulated/0/.usc/maps.db"), Path::Absolute("/storage/emulated/0/.usc/maps.db_" + Shared::Time::Now().ToString() + ".bak"));
...
#ifdef ANDROID
						score.replayPath = Path::Absolute("/storage/emulated/0/.usc/eplays/" + hash + "/" + timestamp.ToString() + ".urf");
#else
						score.replayPath = Path::Normalize(Path::Absolute("/storage/emulated/0/.usc/replays/" + hash + "/" + timestamp.ToString() + ".urf"));
#endif
